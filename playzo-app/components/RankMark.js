...
export default function RankMark({ rank, compact = false, tierLabel = "Tier akun" }) {
  ...
  return (
    <span className={`rank-mark ${tier.className} ${compact ? "rank-mark-compact" : ""}`} title={tier.label}>
      ...
      {!compact && <span className="rank-mark-copy"><strong>{tier.label}</strong><small>{tierLabel}</small></span>}
    </span>
  );
}
